...
Read [Open Knowledge Format v0.1](../references/SPEC.md) completely before interpreting or changing Knowledge Bundle artifacts.

1. **Inspect existing knowledge.** Traverse the relevant `index.md` files and search concepts before choosing a destination.
   - **Complete when:** every plausible existing concept and its relevant index context has been inspected.
2. **Create or incorporate.** Add the knowledge to an existing concept when it belongs there; otherwise create one that satisfies the specification. Keep claims traceable to the user-provided material or explicit external sources.
